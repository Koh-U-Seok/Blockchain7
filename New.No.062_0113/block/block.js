...
class BlockHeader {
  ...
  getDifficulty({
    ...
    adjustmentDifficulty, // 난이도 조절 단위 개수. 이전 세대의 난이도.
    adjustmentTimestamp, // 난이도 조절 단위 개수 이전의 블록의 생성 시간, 이전 세대의 생성 시간.
    DAI, // 난이도 조절 단위 개수. 한 세대에 몇 블록이 들어갔나.
    averageGenerationTime, // 블록 세대당 생성 시간, 블록 10개당 생성 시간, 난이도 조절 기능의 기준.
  }) {
    if (this.height < DAI) {
      this.difficulty = 0;
