...
// 4. reducer의 return 값은 그대로 state에 정의 된다.
//  - useSelector를 사용하면 state 변화 시 렌더링을 다시 해준다.(reRendering)
...
const asyncIncrement = () => {
  ...
  return async (dispatch, getState) => {
    ...
    // 3. thunk에서 해당 함수를 호출한다. action이 객체일 경우에는 reducer에 매개변수로 전달하며 호출한다.
    console.log("3. asyncIncrement return function");
    try {
      // await로 사용한 함수의 호출이 에러가 발생할 수도 있기 때문에 (reject) try, catch 구문을 사용한다.
...
export const reducer = (state = initialize, action) => {
  ...
  switch (type) {
    // 9-1. 현재는 Increment를 호춣중이다.
    case TYPE.INCREMENT:
      // return은 state+1을 state(현재는 count)에 적용한다.
      console.log("10. state = 1", state + 1);
      return state + 1;
