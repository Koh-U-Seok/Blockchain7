...
// 아래 방식으로 작성하는 경우가 많기에 콜백지옥이라고 한다.
...
// Promise는 위와 같은 콜백 지옥을 피하기 위해 사용된다.
// const callbackPromise = (text, time) => {
//   return new Promise((resolve, reject) => {
//     // resolve는 성공 시 결과를 전달한다. 전달할 데이터는 resolve의 매개변수로 전달한다.
//     // reject는 실패 시 에러를전달한다. 사용법은 resolve와 같다.
//     try {
//       // try 스코프 내의 코드를 실행한다.
//       // 도중에 오류가 발생하면 아래의 catch의 error를 전달한다.
//       if (text == "어싱크 확인 2") reject("end");
//       setTimeout(() => {
//         resolve(text);
//       }, time);
//     } catch (error) {
...
//   .then((result) => {
//     // then은 promise 성공 시 실행하는 콜백 함수를 전달한다.
//     // 콜백함수의 매개변수는 76번 줄의 resolve에 전달된 매개변수(text를 result로)를 받는다.
//     console.log(result);
//     return callbackPromise("프라미스 2", 1000);
//     // 다음 프라미스(작업)은 return으로 반환하며 아래의 then으로 위의 프라미스(현재의 89번 줄)의 결과를 받을 수 있다.
//   })
//   .then((result) => {
//     console.log(result);
...
//     console.log(await callbackPromise("어싱크 확인 1", 1000));
//     // await는 프라미스 함수가 끝날 때까지 기다리도록 한다.
//     // 아래의 줄로 내려가지 않는다. 즉, 비동기 작업을 동기처럼 작성할 수 있게 해준다.
//     console.log(await callbackPromise("어싱크 확인 2", 1000));
//     console.log(await callbackPromise("어싱크 확인 3", 1000));
//     console.log(await callbackPromise("어싱크 확인 4", 1000));
//     console.log(await callbackPromise("어싱크 확인 5", 1000));
//   } catch (error) {
//     console.log(error);
