    ...
    <div className="App">
      ...
      <button
        onClick={() => {
          promiseTime(action.increment(), 1000).then((data) => {
            // then 메서드를 사용해서 매개변수로 콜백함수를 전달한다.
      ...
      <button
        onClick={async () => {
          ...
          // 중간 과정으로 Promise, axion 등을 처리할 수 있도록 async, await를 사용할 수 있게 추가한다.
        }}
      >
        Promise -
      </button>
      <button
        onClick={() => {
          // 1. 사용자가 클릭한다. 클릭 시 dispatch를 호출하면 action의 asyncIncrement 메서드를 호출한다. 해당 메서드는 함수를 반환(return)한다.
          console.log("1. Click");
