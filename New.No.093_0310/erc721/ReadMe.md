...
```
    ...
    - URI는 Uniform Resource Identifier의 약자로 특정 데이터를 식별하는 식별자, 리소스를 구분하는 고유 문자열
        - URI : 데이터를 구분한다. 데이터를 구분할 때 사용하는 문자열 | URL : 데이터의 위치를 가리킨다. 데이터가 어디있는지 나타낸다.
...
interface IERC721 {
  ...
  function ownerOf(uint _token) external view returns (address owner);
  ...
  // 토큰 하나에 대한 권한 위임. 모든 토큰에 대해 권한을 위임하지 않고 몇 개만 권한을 위임한다고 하면 그 횟수 만큼 권한 위임을 반복해야 한다.

  function setApprovalForAll(address _operator, bool _approved) external;

  // 모든 토큰에 대한 권한 위임(취소할 수도 있다.)

  function getApproved(uint _tokenId) external view returns (address operator);

  // 토큰에 대한 권한을 위임받은 지갑(대리인)
...
contract ERC721 is IERC721, IERC721Metadata {
  ...
  // IERC721Metadata에서 선언되었기 때문에 override해야 한다.
  ...
  function ownerOf(uint _tokenId) public view override returns (address) {
    ...
    require(owner != address(0), "E RC721 : invalid tokenId");
    return owner;
  }
  ...
  // 토큰 보내는 메서드, _from => _to

  function approve(address _to, uint _tokenId) external override {
    address owner = _owners[_tokenId];
    // address owner = ownerOf(_tokenId); << 수수료가 필요하기 때문에 이렇게 쓰면 절대 안된다.
    // 위임할 토큰의 주인 확인
    ...
    require(
      ...
      "ERC721 : approve caller is not token owner of approved for all"
    );
  ...
  // 트랜잭션 보낸 계정의 모든 토큰에 대한 권한을 _operator에게 _approved로 설정한다. | _apperoved == true << 모든 권한 위임 | _approved == false << 권한 위임 취소
  ...
  function _isApproveOrOwner(
    ...
    return (_spender == owner ||
      ...
      // from이 토큰에 대해 모든 권한을 갖고 있는 대리인인가?
  ...
  // tokenURI 메서드는 상속받아서 override 했지만 다시 상속시켜서 재정의할 것이다. => virtual 옵션 추가
...
contract ERC721Enumerable is ERC721 {
  ...
  function mint(address _to) public {
    // 계정 주소만 받아서 minting, tokenId는 자동 생성(현재 토큰의 갯수)
