...
const server = net.createServer((client) => {
  ...
  client.on("data", (data) => {
    ...
    // Sec-Fetch-Site: none << CORS 설정 : 해당 정보를 요청한 주소가 서버의 주소와 맞는가? 서버의 데이터를 조금이라도 안전하게 지키기 위해서 확인한다.
...
Hi Block 7`);
    ...
    // Content-Type : 어떤 데이터로 응답하거나, 어떤 정보를 보낼지 포멧(확장자 등등)을 설정, charset은 언어 포멧 설정
    // Content-Length: 데이터으 ㅣ길이
    // << 줄바꿈 한번 있어야 한다.
    // 보낼 데이터
  });
});
