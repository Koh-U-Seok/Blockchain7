...
const Counter = ({ web3, account }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // );
      // const CounterContract = await axios.post("http://localhost:8080/api/CC");
      const CounterContract = (await axios.post("http://localhost:8080/api/CC"))
        .data.CC;
      ...
      web3.eth.subscribe("logs", { address: CA }).on("data", (log) => {
        ...
        // Solidity에서 event 이벤트명(로그를 남길 데이터, 변수)를 선언하고 로그를남길 순간에 emit으로 구독한 서버에 알린다.
        // emit으로 전달될 데이터는 log.data에 들어있다.
        // subscribe의 두 번째 매개변수에 옵션을 추가할 수 있으며 address 옵션은 해당 주소에 대해서만 logs를 받는다.
        console.log("log : ", log);
