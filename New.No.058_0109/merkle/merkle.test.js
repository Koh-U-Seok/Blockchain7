...
const {
  ...
  oddThirdRoot,
  oddMerkleRoot,
} = require("./oddMerkle.js");

// jest를 사용하는 방법
//  - .js파일을 .test.js 파일로 생성
//  - npx jest를 실행하면 .test.js 파일을 모두 확인한다.
//      - 또는 package.json 파일 내의 test 명령어를 jest로 수정한다.
//          - "test" : "jest"
...
describe("이것은 테스트다.", () => {
  ...
  // test || it
  // it(name, func)
  // it은 또는 test는 name이라는 이름의 테스트를 실행한다.
  // 해당 테스트의 구현 내용(코드)는 func(함수)에 포함된다.

  // expect(확인할 내용) : 적혀있는 그대로 확인할 함수, 객체, 변수 등등을 매개변수로 전달한다. 이것만 있으면(뒤에 붙는 toBe()등이 없으면) 아무 의미가 없다.
  // toBe(비교 대상) : 정확히 일치하는지를 확인한다.
  //  - toXXXXXXXX : 를 매쳐(matcher)라고 부른다. << 엄청 많다.
