...
async function getBalance(_account) {
  ...
  return parseInt(parseInt(result, 16) / Math.pow(10, 15)) / 1000;
}

async function getWallet(_account) {
  if (interval !== undefined) mineStop();
  accountElem.innerHTML = _account;
  const balance = await getBalance(_account);
  ...
  return { account: _account, balance: balance };
}
...
document.forms["select-meta"].onsubmit = async function (e) {
  e.preventDefault();
  const { account, balance } = await getWallet(e.target["meta"].value);
