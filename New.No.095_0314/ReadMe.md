...
contract nftToken is ERC721Enumerable, Ownable {
  ...
  // 상수의 변수명을 정할 때 전부 대문자로 하곤 한다.(업계 국룰)

  uint public mint_price = 1 ether;
  // 민팅 가격, 사용자가 NFT를 올릴 때마다 1 이더 씩 받는다.
  // ether, gwei, secont, minute, day << 단위를 사용할 수 있다.
  // ether 경우 10 ** 18

  string public metadataURI;
  // NFT의 tokenID값에 매칭되는 tokenURI의 앞부분
  ...
  //      - https://docs.opensea.io/docs/metadata-standard#attributes
  ...
  function tokenURI(
    ...
    // tokenURI를 생성 메서드
    // ERC721에 virtual 옵션을 포함하여 구현되어 있음
    ...
    // NFT에 대한 데이터를 저장한 URL 주소를 찾아 데이터를 받아올 수 있도록 구현되었다.
  }
  ...
  function mintToken() public payable {
    ...
    // 생성 시 이더를 받고(msg.value) 가격을 확인한다. 돈을 받고 NFT를 만들어준다.
    ...
    // Rank와 Type을 기준으로 토큰 수량 정리

    payable(Ownable.owner()).transfer(msg.value);
    // 받은 이더를 컨트랙트 소유자에게 전달(NFT 토큰 컨트랙트 등록자)
    ...
    function getRandomTokenData(
        ...
        // 그래서 유일한 값인 tokenId를 가져와서 암호화한 후 나머지 연산으로 0~99까지 랜덤한 수를 만든다.
...
contract SaleToken {
  ...
  function PurchaseToken(uint _tokenId) public payable {
    ...
    // 현재 컨트랙트가 NFT 소유자에게 받은 이더를 전달한다.
...
- 민팅 => NFftToken의 mintToken을 호출한다.(이더도 보내야 한다.)
  -> require로 확인 후에 NftToken 컨트랙트의 getRandomTokenData 메서드로 토큰 정보를 만든다.
  -> \_mint 메서드로 NFT 추가
...
- 웹페이지에서 판매 NFT 목록 출력 => SaleToken 컨트랙트의 getSaleTokenList 메서드를 호출한다.
  ...
  - 출력해서 보여주어야 다른 사용자들이 판매 또는 구매 등등 가능하다.

- 웹페이지에서 내 소유의 NFT 목록 출력 => 메타마스크의 지갑 계정 주소를 기준으로 saleToken 컨트랙트의 getOwnerTokens 메서드를 호출한다.

  - 이후 내용은 판매 NFT 목록 출력과 같다.

- 구현 시나리오에 따라 다르지만 만약 민팅 후에 NFT 정보 페이지로 이동 또는 등록한 NFT 정보를 모달창으로 출력하여 확인할 경우 getLatestToken 메서드를 사용하게 된다.
